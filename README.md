...
### 다중 채팅 서버간 채팅 메시지 공유
이번에는 STOMP를 제거하고 WebSocket만으로 통신하며, Kafka 메시징 시스템을 통해 채팅 메시지를
여러 서버가 실시간으로 공유하는 구조를 구현할 예정이다. 따라서 채팅 메시지가 수신되면 우선 Kafka 토픽에
...
Redis는 eventual consistency 특성상 쓰기 전 정합성 검증을 위한 조회 용도로는 적합하지 않다.
데이터 불일치 상태에서 쓰기가 발생할 수 있기 때문이다. 따라서 Redis를 읽기 캐시로 사용하는 로직은 구현하지 않으며,  
...
lag이 쌓인 상태에서 문제가 발생했을 경우, 문제를 일으킨 컨슈머 프로세스(웹 서버)를 재실행하면 브로커와의
연결이 재수립되며, 마지막 커밋 오프셋부터 메시지가 다시 소비되는 것을 확인하였다.
