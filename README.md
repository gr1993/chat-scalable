...
이번 프로젝트를 진행하면서 성능 면에서 가장 큰 제한을 느낀 부분은 Kafka 클러스터였다. 동시 접속자가 100명을  
넘어가면 클러스터 동기화에 부하가 발생하면서, 채팅 메시지 토픽의 Consumer들에게 lag이 생기기 시작했다.  
또한 Kafka 노드에서 동기화 오류가 발생하면 다른 노드를 찾지 못하는 문제가 생기면서 lag가 계속 쌓이고, 레코드가  
전혀 소비되지 않는 현상이 발생했다.  
